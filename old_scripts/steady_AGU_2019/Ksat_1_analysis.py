...
mean_drainage_densities = np.zeros((len(Ks_all),3))
recession_k = np.zeros((len(Ks_all),2))
IDs = np.zeros(len(Ks_all))
Ks_save = np.zeros(len(Ks_all))
paths = glob.glob('../DupuitLEMResults/vary_Ksat_1*')
...
for i in range(len(paths)):
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/elev_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/soil_'+str(ID) +'.png')
    plt.close()

    # relative saturation
    plt.figure(figsize=(8,6))
    imshow_grid(grid,(wt-base)/(elev-base), cmap='Blues', limits=(0,1), colorbar_label = 'Relative saturated thickness [-]', grid_units=('m','m'))
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/rel_thickness_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/surface_water_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/cross_section_'+str(ID) +'.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/log_rel_change_'+str(ID)+'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/recession_time_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/recession_channels_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/recession_plot_'+str(ID) +'.png', bbox_inches = 'tight')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/steady_channels_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Ksat_1/min_channels_'+str(ID) +'.png')
    plt.close()

data = {'ID':IDs, 'Ks':Ks_save, 'DD_steady':mean_drainage_densities[:,0], 'DD_1':mean_drainage_densities[:,1], 'DD_30':mean_drainage_densities[:,2], 'rec_a':recession_k[:,0], 'rec_c':recession_k[:,1] }
df = pd.DataFrame(data)
pickle.dump(df,open('../DupuitLEMResults/figs/vary_Ksat_1/data_processed.p','wb'))
