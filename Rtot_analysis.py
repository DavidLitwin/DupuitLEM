...
for i in range(len(paths)):
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/elev_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/soil_'+str(ID) +'.png')
    plt.close()

    # relative saturation
    plt.figure(figsize=(8,6))
    imshow_grid(grid,(wt-base)/(elev-base), cmap='Blues', limits=(0,1), colorbar_label = 'Relative saturated thickness [-]', grid_units=('m','m'))
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/rel_thickness_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/surface_water_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/cross_section_'+str(ID) +'.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/log_rel_change_'+str(ID)+'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/recession_time_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/recession_channels_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/recession_plot_'+str(ID) +'.png', bbox_inches = 'tight')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/steady_channels_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_Rtot/min_channels_'+str(ID) +'.png')
    plt.close()

data = {'ID':IDs, 'R_tot':R_tot_save, 'DD_steady':mean_drainage_densities[:,0], 'DD_1':mean_drainage_densities[:,1], 'DD_30':mean_drainage_densities[:,2], 'rec_a':recession_k[:,0], 'rec_c':recession_k[:,1] }
df = pd.DataFrame(data)
pickle.dump(df,open('../DupuitLEMResults/figs/vary_Rtot/data_processed.p','wb'))
