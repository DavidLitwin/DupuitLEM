...
Analysis of the output of vary_duration. Make static plots, calculate drainage
...
mean_drainage_densities = np.zeros((len(dt_event_all),2))
recession_k = np.zeros((len(dt_event_all),2))
IDs = np.zeros(len(dt_event_all))
dt_save = np.zeros(len(dt_event_all))
paths = glob.glob('../DupuitLEMResults/vary_duration*')
...
for i in range(len(paths)):
    ...
    dt_save[i] = dt_event_all[ID]/3600
    dt_event = dt_event_all[ID]
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_duration/elev_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_duration/soil_'+str(ID) +'.png')
    plt.close()

    # relative saturation
    plt.figure(figsize=(8,6))
    imshow_grid(grid,(wt-base)/(elev-base), cmap='Blues', limits=(0,1), colorbar_label = 'Relative saturated thickness [-]', grid_units=('m','m'))
    plt.savefig('../DupuitLEMResults/figs/vary_duration/rel_thickness_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_duration/surface_water_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_duration/cross_section_'+str(ID) +'.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('../DupuitLEMResults/figs/vary_duration/log_rel_change_'+str(ID)+'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_duration/max_channels_'+str(ID) +'.png')
    plt.close()

    plt.figure()
    imshow_grid(grid,interevent_channels, plot_name='Minimum channel extent', allow_colorbar=False, cmap='Blues', grid_units=('m','m'))
    plt.savefig('../DupuitLEMResults/figs/vary_duration/min_channels_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_duration/recession_time_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_duration/recession_channels_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_duration/recession_plot_'+str(ID) +'.png', bbox_inches = 'tight')
    plt.close()

data = {'ID':IDs, 'dt_event':dt_save, 'DD_max':mean_drainage_densities[:,0], 'DD_min':mean_drainage_densities[:,1], 'rec_a':recession_k[:,0], 'rec_c':recession_k[:,1] }
df = pd.DataFrame(data)
pickle.dump(df,open('../DupuitLEMResults/figs/vary_duration/data_processed.p','wb'))
