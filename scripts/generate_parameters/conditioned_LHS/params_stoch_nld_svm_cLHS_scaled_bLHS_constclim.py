...
(K, D, U) effects from climate effects:
- b (regolith thickness) is sampled as an independent LHS parameter, over the same
  approximate range that b = 2*sqrt(hg) produces in the base script, rather than
  being coupled to hg (see params_stoch_nld_svm_cLHS_scaled_bLHS.py).
- P and PET are fixed constants (P=1 m/yr, PET=0.5 m/yr) instead of drawn from the
  real-watershed cLHS data, removing climate variability across the sample. U, K, D
  keep the same ranges/sampling as the base script.
