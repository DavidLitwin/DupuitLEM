"""
Comparison variant of params_stoch_nld_svm_cLHS_scaled.py, to isolate subsurface
(K, D, U) effects from climate effects while keeping the b-hg coupling: P and PET
are fixed constants (P=1 m/yr, PET=0.5 m/yr) instead of drawn from the real-watershed
cLHS data, removing climate variability across the sample. U, K, D, and b (still
b = 2*sqrt(hg), coupled to relief as in the base script) are unchanged. Compare
against params_stoch_nld_svm_cLHS_scaled_bLHS_constclim.py (same constant climate,
but b sampled independently) to isolate the effect of the b-hg coupling under fixed
climate.

Generate parameters for StreamPowerModel from conditioned latin hypercube sampling on
U (erosion rate) from global watersheds; P and PET are held constant (see above).
...
1. Forcing: U from data (external, real watersheds); P, PET fixed constants.
2. Secondary material parameters (sampled via LHS, independent of U): K, D, T
...
bnds = list(zip(bnd_logK, bnd_logD, bnd_logT, bnd_logds, bnd_logrho))

sampler = qmc.LatinHypercube(d=len(bnds[0]), seed=2023)  # K, D, T, ds, rho
...
b = 2*np.sqrt(hg)  # regolith thickness scales with characteristic relief

T = 10**scaled_sample[:, 2]
ds = 10**scaled_sample[:, 3]
rho = 10**scaled_sample[:, 4]
ksat = T/b

P_const = 1.0    # m/yr -- fixed climate, to isolate subsurface (K, D, U) effects
PET_const = 0.5  # m/yr
p = np.full(len(dfc), P_const / (3600*24*365))    # m/s
PET = np.full(len(dfc), PET_const / (3600*24*365))  # m/s
pet = PET/(1-rho)
ai = pet/p
tr = rho * (ds/p)
tb = (1-rho) * (ds/p)
