...
# prod = np.array(list(product(K_all, D_all, v0_all, m_all)))
# df_params = pd.DataFrame(prod, columns=['K', 'D', 'v0', 'm'])

prod = np.array(list(product(ksn_a_all, D_all, v0_all, m_all)))
df_params = pd.DataFrame(prod, columns=['ksn_a_pred', 'D', 'v0', 'm'])

n_from_m = {0.5:1, 0.8:2.0, 0.4:0.6}
df_params['n'] = df_params['m'].map(n_from_m)
df_params['U'] = U
df_params['Ka'] = df_params['U']/df_params['ksn_a_pred']**df_params['n']
df_params['K'] = df_params['Ka']/df_params['v0']**df_params['m']
...
# df_params['ksn_a_pred_spld'] = (df_params['ksn_pred'] * df_params['v0']**(-df_params['m']/df_params['n'])) * (df_params['lg/v0']+1)**(1/df_params.n)
...
# dfg = df_params.groupby(['m', 'ksn_a_pred'])
...
#         ax[i].plot(g1['D'], g1['ksn_a_pred_spld'], linestyle='-', color=colors1[j])
#         ax[i].scatter(g1['D'], g1['ksn_a_pred_spld'], s=10, label=f'$v_0$={v0:.2f}', color=colors1[j])
#         j+=1
