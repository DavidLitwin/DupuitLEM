...
calc_sigma = lambda b, ne, p, tr, tb: (b * ne) / (p * (tr + tb))
calc_rho = lambda tr, tb: tr / (tr + tb)
calc_ai = lambda pet, p, tr, tb: (pet * tb) / (p * (tr + tb))
calc_phi = lambda ne, na: na / ne
...
df_params['tb'] = 22.58 * 3600 # s
df_params['ds'] = 6.86 / 1000 # m
df_params['p'] = df_params['ds']/(df_params['tr']+df_params['tb']) # m/s
df_params['pet'] = 1 / (3600 * 24 * 365) # m/s
...
df_params['sigma'] = calc_sigma(df_params.b, df_params['ne'], df_params.p, df_params.tr, df_params.tb)
df_params['rho'] = calc_rho(df_params.tr, df_params.tb)
df_params['ai'] = calc_ai(df_params.pet, df_params.p, df_params.tr, df_params.tb)
df_params['phi'] = calc_phi(df_params['ne'], df_params.na)
