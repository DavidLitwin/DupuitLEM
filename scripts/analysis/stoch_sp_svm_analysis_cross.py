...
ID = int(task_id)
base_folder = os.environ['BASE_OUTPUT_FOLDER']
...
plt.title('params ID %d, grid ID %d, Iteration %d'%(ID,grid_id,iteration))
plt.savefig('./post_proc/%s/%s/elev_%d_x_%d.png'%(base_folder, cross_folder, grid_id, ID))
plt.close()


########## Run hydrological model
# load parameters and save just this ID (useful because some runs in a group have been redone with diff parameters)
df_params = pd.read_csv('./%s-%d/parameters.csv'%(base_folder, ID), index_col=0)[task_id]
...
df = pd.read_csv('./post_proc/%s/%s/dt_qs_s_%d.csv'%(base_folder, cross_folder, ID), sep=',',header=None, names=['dt','r', 'qs', 'S', 'sat_nodes'])
...
filename = './post_proc/%s/%s/grid_%d_x_%d.nc'%(base_folder, cross_folder, grid_id, ID)
...
df_output.to_csv('./post_proc/%s/%s/output_%d_x_%d.csv'%(base_folder, cross_folder, grid_id, ID))
df.to_csv('./post_proc/%s/%s/q_s_dt_%d_x_%d.csv'%(base_folder, cross_folder, grid_id, ID))
