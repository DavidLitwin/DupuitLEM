...
mg = from_netcdf(path) # to_netcdf and from_netcdf preserve boundary condtions
...
for i in range(N):
    ...
    if i%output_interval==0  or i == max(range(N)):
        ...
        filename = base_path + 'ssrun_%d_grid_%d.nc'%(ID,i)
        to_netcdf(mg, filename, include="at_node:topographic__elevation")

        # open previous saved file, find max rate of change
        filename0 = base_path + 'ssrun_%d_grid_%d.nc'%(ID,i-output_interval)
...
output["base_output_path"] = base_path + 'postrun_'
output["run_id"] = ID #make this task_id if multiple runs

postrun_ss_cond = {}
postrun_ss_cond['stop_at_rate'] = 1e-3*U
postrun_ss_cond['how'] = 'percentile'
postrun_ss_cond['percentile_value'] = 100
