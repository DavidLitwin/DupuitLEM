...
stochastic stream power components across multiple timesteps.

For each chosen timestep, it performs the following analyses:
...
Outputs spatial fields and scalar metrics for each model run ID and timestep.
"""
...
def analyze_time(t_index):
    load_fields_from_dataset(xr_ds, grid, t_index=t_index, last_non_nan=False)
    index = t_index
    ...
    plt.savefig(os.path.join('..', 'post_proc', base_output_path, f'elev_ID_{ID}_t{t_index}.png'))
    plt.close()
...
#initialize components
    ...
    f = open('../post_proc/%s/dt_qs_s_%d_t%d.csv'%(base_output_path, ID, t_index), 'w')
    ...
    timeseries_path = os.path.join('..', 'post_proc', base_output_path, f'dt_qs_s_{ID}_t{t_index}.csv')
    ...
    filename = os.path.join(base_file_name, f'grid_{ID}_t{t_index}.nc')
    ...
    df_output = pd.DataFrame.from_dict(df_output, orient='index', columns=[f'{ID}_t{t_index}'])
    df_output.to_csv(os.path.join(base_file_name, f'output_ID_{ID}_t{t_index}.csv'))
    df.to_csv(os.path.join(base_file_name, f'q_s_dt_ID_{ID}_t{t_index}.csv'))

# Loop over specified time indices
with concurrent.futures.ProcessPoolExecutor(max_workers=min(len(t_indices), 4)) as executor:
    list(executor.map(analyze_time, t_indices))
