...
def D_fun(Lh, U, psi, stoe):
    return (Lh*U)/((1 + psi)*stoe)

def Sc_fun(psi, stoe):
    return stoe*np.sqrt(1 + psi)/np.sqrt(psi)
...
def generate_parameters(U, lg, p, n, psi, stoe, gam, hi, lam, sigma, rho, ai):
    ...
    return D, U, hg, lg, Lh, Sc, stoe, ksat, p, pet, b, ds, tr, tb, n, alpha, psi, gam, hi, lam, sigma, rho, ai

psi_all = np.geomspace(0.01, 10, 13) # kappa = alpha*lam = hg/lg^2 * Lh
