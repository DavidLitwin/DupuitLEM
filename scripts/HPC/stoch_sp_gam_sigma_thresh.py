...
describe storage vs storm depth and rainfall steadiness respectively. Also
includes a streampower threshold, and threshold parameter theta.
...
def generate_parameters(p, n, v0, hg, lg, tg, gam, hi, sigma, rho, theta):
    ...
    return K, D, U, ksat, p, b, n, v0, hg, lg, tg, E0, ds, tr, tb, gam, hi, sigma, rho, theta
...
params = np.zeros((len(sigma1),20))
for i in range(len(sigma1)):
    params[i,:] = generate_parameters(p, n, v0, hg, lg, tg, gam1[i], hi, sigma1[i], rho, theta)

df_params = pandas.DataFrame(params,columns=['K', 'D', 'U', 'ksat', 'p', 'b', 'n', 'v0', 'hg', 'lg', 'tg', 'E0', 'ds', 'tr', 'tb', 'gam', 'hi', 'sigma', 'rho', 'theta'])
...
#use surface_water_area_norm__discharge (Q/sqrt(A)) for Theodoratos definitions. Note that we don't include incision threshold because this is handled by HydrologyEventThresholdStreamPower model.
