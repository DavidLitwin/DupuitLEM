...
def generate_parameters(p, n, v0, hg, lg, tg, gam, hi, beta, rho, ai):
    ...
    return K, D, U, ksat, p, pet, b, n, v0, hg, lg, tg, ds, tr, tb, gam, hi, beta, rho, ai
...
params = np.zeros((len(beta1),20))
for i in range(len(beta1)):
    params[i,:] = generate_parameters(p, n, v0, hg, lg, tg, gam1[i], hi1[i], beta1[i], rho, ai)

df_params = pandas.DataFrame(params,columns=['K', 'D', 'U', 'ksat', 'p', 'pet', 'b', 'n', 'v0', 'hg', 'lg', 'tg', 'ds', 'tr', 'tb', 'gam', 'hi', 'beta', 'rho', 'ai'])
...
output["base_output_path"] = './data/stoch_sp_svm_gam_beta_'
...
# use surface_water_area_norm__discharge (Q/sqrt(A))
