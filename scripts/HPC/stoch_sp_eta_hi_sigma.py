...
eta = gamma/Hi, and extended using parameters Sigma and Rho that
...
def ds_fun(hg, n, eta, sigma):
    return (hg*n*eta)/sigma

def tr_fun(hg, p, n, eta, sigma, rho):
    return (hg*n*eta*rho)/(p*sigma)

def tb_fun(hg, p, n, eta, sigma, rho):
    return (hg*n*eta)*(1-rho)/(p*sigma)

#generate dimensioned parameters
def generate_parameters(p, n, v0, hg, lg, tg, eta, hi, sigma, rho):
    ...
    ds = ds_fun(hg, n, eta, sigma)
    tr = tr_fun(hg, p, n, eta, sigma, rho)
    tb = tb_fun(hg, p, n, eta, sigma, rho)

    return K, D, U, ksat, p, b, n, v0, hg, lg, tg, ds, tr, tb, eta, hi, sigma, rho
...
output["base_output_path"] = './data/stoch_sp_eta_hi_sigma_'
