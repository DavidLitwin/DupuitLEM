...
eta = gamma/Hi, and extended using parameters Beta and Rho that
...
def generate_parameters(p, n, v0, hg, lg, tg, eta, hi, beta, rho):
    ...
    return K, D, U, ksat, p, b, n, v0, hg, lg, tg, ds, tr, tb, eta, hi, beta, rho


#parameters
eta_all = np.geomspace(0.1, 50, 5)
hi_all = np.geomspace(0.05, 5, 5)
beta_all = np.array([0.01, 0.1])
beta1 = np.array(list(product(beta_all, hi_all, eta_all)))[:,0]
hi1 = np.array(list(product(beta_all, hi_all, eta_all)))[:,1]
eta1 = np.array(list(product(beta_all, hi_all, eta_all)))[:,2]
rho = 0.1
...
for i in range(len(beta1)):
    params[i,:] = generate_parameters(p, n, v0, hg, lg, tg, eta1[i], hi1[i], beta1[i], rho)
...
output["base_output_path"] = './data/stoch_sp_eta_hi_beta_'
