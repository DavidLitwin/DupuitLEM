...
The purpose here is to determine how landscape properties change when grid cell
width v0 is changed. Keep all else the same. Steady hydrology equivalent of 
simple_lem_4.
...
params = np.zeros((len(v0_all),14))
for i in range(len(v0_all)):

    params[i,:] = generate_parameters(p1, n1, a0, v0_all[i], hg, lg, tg, gam1, lam1)
...
K = df_params['K'][ID]
a0 = df_params['a0'][ID]
v0 = df_params['v0'][ID] #min contour width (grid spacing) [m]
Nx = int(Nx_all[ID]) #grid dimension
