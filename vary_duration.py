...
Test different durations of event and interevent
...
for i in range(N):
    ...
    if i % output_interval == 0:
        ...
        filename = './data/vary_duration_' + Ks_print + '_grid_' + str(i) + '.nc'
        write_raster_netcdf(
                filename, grid, names=output_fields, format="NETCDF4")
        print('Completed loop %d' % i)

        filename = './data/vary_duration_' + Ks_print + '_substeps' + '.txt'
        np.savetxt(filename,num_substeps)

        filename = './data/vary_duration_' + Ks_print + '_max_rel_change' + '.txt'
        np.savetxt(filename,max_rel_change)

        filename = './data/vary_duration_' + Ks_print + '_90perc_rel_change' + '.txt'
...
filename = './data/vary_duration_' + Ks_print + '_time' + '.txt'
