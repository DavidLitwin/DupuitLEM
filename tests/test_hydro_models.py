...
def test_steady_sp_hex():
    ...
    gdp = GroundwaterDupuitPercolator(mg, recharge_rate=1e-6)
    hm = HydrologySteadyStreamPower(mg, groundwater_model=gdp, routing_method="Steepest")
...
def test_stoch_sp_raster():
    ...
    gdp = GroundwaterDupuitPercolator(mg, recharge_rate=1e-6)
    pd = PrecipitationDistribution(mg, mean_storm_duration=10,
        mean_interstorm_duration=100, mean_storm_depth=1e-3,
        total_t=100)
...
def test_stoch_sp_hex():
    ...
    pd = PrecipitationDistribution(mg, mean_storm_duration=10,
        ...
        total_t=100)
    pd.seed_generator(seedval=1)
    hm = HydrologyEventStreamPower(mg, precip_generator=pd, groundwater_model=gdp, routing_method="Steepest")
    ...
    assert_almost_equal(hm.q_an[4], 0.00017614*np.sqrt(3)/2 / np.sqrt(np.sqrt(3)/2*100))
...
def test_stoch_sp_raster_record_state():
    ...
    times = np.array([0.0, 
                     ...
                     hm.storm_dts[0]+hm.interstorm_dts[0]+hm.storm_dts[1]+hm.interstorm_dts[1],
                     ])
