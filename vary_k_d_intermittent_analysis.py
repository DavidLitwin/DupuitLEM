...
Analysis of the output of regular intermittent vary_k_d. Make static plots, calculate drainage
...
paths = glob.glob('../DupuitLEMResults/vary_k_d/vary_k_d_9vals-*')
...
for i in range(len(paths)):
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/elev_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/soil_'+str(ID) +'.png')
    plt.close()

    # relative saturation
    plt.figure(figsize=(8,6))
    imshow_grid(grid,(wt-base)/(elev-base), cmap='Blues', limits=(0,1), colorbar_label = 'Relative saturated thickness [-]', grid_units=('m','m'))
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/rel_thickness_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/surface_water_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/cross_section_'+str(ID) +'.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/log_rel_change_'+str(ID)+'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/max_channels_'+str(ID) +'.png')
    plt.close()

    plt.figure()
    imshow_grid(grid,interevent_channels, plot_name='Minimum channel extent', allow_colorbar=False, cmap='Blues', grid_units=('m','m'))
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/min_channels_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/recession_time_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/recession_channels_'+str(ID) +'.png')
    plt.close()
    ...
    plt.savefig('../DupuitLEMResults/figs/vary_k_d_intermittent/recession_plot_'+str(ID) +'.png', bbox_inches = 'tight')
    plt.close()
...
pickle.dump(df,open('../DupuitLEMResults/figs/vary_k_d_intermittent/data_processed.p','wb'))
