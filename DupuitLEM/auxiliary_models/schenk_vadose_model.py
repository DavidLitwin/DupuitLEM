...
class SchenkVadoseModel:
    ...
    def __init__(self,
        ...
        self.depths = np.linspace(self.b/self.Nz,self.b,self.Nz) # bottom elev of each bin
        self.sat_profile = np.zeros_like(self.depths) # saturation state (binary) in each bin
    ...
    def run_event(self, storm_depth):
        ...
        self.recharge_at_depth[:] = (n_to_fill - np.cumsum(self.sat_diff))*self.bin_capacity
        self.sat_diff[:] = 0
